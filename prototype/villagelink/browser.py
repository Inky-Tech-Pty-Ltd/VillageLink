...
function composeVillageLink(){const domain=document.getElementById('domain').value.trim().toLowerCase().replace(/\.$/,'');const a=document.getElementById('endpoint-a').value.trim(),b=document.getElementById('endpoint-b').value.trim(),message=document.getElementById('message'),result=document.getElementById('result');message.textContent='';if(!domain||domain.includes('/')||domain.includes(':')||domain.includes(' ')){message.textContent='Enter a bare domain such as village.link.';return}if(!a||!b){message.textContent='Enter both URLs.';return}const composed=markerForDomain(domain)+encodeEndpoint(a)+'!'+encodeEndpoint(b);const link=document.createElement('a');link.href=composed;link.textContent=composed;link.title=composed;result.replaceChildren(link);result.style.display='block'}
function composeStarCredential(){const domain=document.getElementById('domain').value.trim().toLowerCase().replace(/\.$/,'');const a=document.getElementById('endpoint-a').value.trim(),entered=[...document.querySelectorAll('.b-endpoint')].map(input=>input.value.trim()).filter(Boolean),message=document.getElementById('message'),result=document.getElementById('result');message.textContent='';if(!domain||domain.includes('/')||domain.includes(':')||domain.includes(' ')){message.textContent='Enter a bare domain such as village.link.';return}if(!a){message.textContent='Enter the centre URI A.';return}if(!isUri(a)){message.textContent='A must be a URI with a scheme, such as https:, mailto: or urn:.';return}const invalid=entered.find(value=>!isUri(value));if(invalid){message.textContent='Every B must be a URI with a scheme.';return}const members=[...new Set(entered)].sort(),credential={type:'StarCredential',domain:domain,A:a,B:members},pre=document.createElement('pre');pre.textContent=JSON.stringify(credential,null,2);result.replaceChildren(pre);result.style.display='block'}
