# ADR-005: Adopt conventional URI prefixes for endpoint identifiers

**Status:** Accepted  
**Date:** 4 September 2026  
**Prefix conventions adopted:** 6 September 2026

## Context

The Village Link primitive connects two independently meaningful identifiers, A and B. Existing online memory systems often already provide suitable URIs, but Village Link must also be able to refer to nouns in memory systems that do not already have an online namespace: for example, a person as remembered at `Family Christmas`, or a person visible in a particular collection of security-camera records.

The project has developed two useful prefix conventions:

- `gc` — **global context**, intended to identify a noun without restriction to a particular memory-system context; and
- `vc` — **village context**, intended to identify a noun within a contextual namespace.
...
The project adopts `gc.` and `vc.` as conventional DNS prefixes for newly minted endpoint identifiers:

- **`gc.` — global context**
- **`vc.` — village context**

These prefixes are conventions carried within ordinary HTTPS/DNS naming, not new URI schemes and not privileged endpoint types in the core primitive.

Examples minted beneath the reference domain include:
...
Other domain owners may adopt the same prefix conventions independently, for example:
...
They may also mint identifiers under other namespaces. Village Link does not require permission from, registration with, or dereferencing through `village.link` for an endpoint URI to participate in the graph.

## Consequences

The core remains small: A and B are URIs. Existing URIs may be used directly; the `gc.` and `vc.` conventions are available when a suitable endpoint URI does not already exist.
...
Because `gc.` is a naming convention rather than an authority claim, multiple publishers may mint competing global-context identifiers for what they believe to be the same noun. Their equivalence, truth and weight remain matters for evidence, graph structure and consuming systems.
...
ADR-001 establishes the self-contained two-ended Village Link primitive. This ADR specifies the treatment of endpoint identifiers A and B without enlarging that primitive.
...
ADR-006 adopts `wab.` as the corresponding marker prefix for the two-ended Village Link form.